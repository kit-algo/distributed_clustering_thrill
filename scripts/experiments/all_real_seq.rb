...
jobs.each do |job|
  ...
  if include_mapeq
    ...
    puts `msub -v GRAPH=#{graph} -v CLUSTERING=clusterings/infomap_directed -l walltime=#{"%02d" % (hours * 4)}:#{"%02d" % (minutes * 2)}:00 #{ENV['HOME']}/code/scripts/moab/seq_infomap_directed.sh`
