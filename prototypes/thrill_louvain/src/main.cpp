...
thrill::DIA<NodeInfo> louvain(thrill::DIA<EdgeType>& edge_list) {
  ...
  auto node_clusters = edge_list
    ...
    //   [](const std::pair<uint32_t, Edge> & edge_with_partition) { return edge_with_partition.first; },
    //   [](auto &, const uint32_t &) {
    ...
    //   [](std::vector<std::pair<uint32_t, uint32_t>> & mapping, const auto emit) {
  ...
  auto new_cluster_ids_times_size = meta_clustering
    .Zip(cluster_sizes, [](const NodeInfo & meta_node_cluster, const std::pair<uint32_t, uint32_t> & cluster_size) { return std::make_pair(meta_node_cluster.data, cluster_size.second); }) // assert(meta_node_cluster.id == cluster_size.first)
    .template FlatMap<uint32_t>(
